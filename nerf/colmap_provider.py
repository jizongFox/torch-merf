...
class ColmapDataset:
    def __init__(self, opt, device, type="train", n_test=24):
        ...
        if self.colmap_path is None:
            raise ValueError(
                f"Cannot find colmap sparse output under {self.root_path}, please run colmap first!"
            )
        ...
        print(
            f"[INFO] {exist_mask.sum()} image exists in all {exist_mask.shape[0]} colmap entries."
        )
        ...
        print(
            f"[INFO] ColmapDataset: load poses {self.poses.shape}, points {self.pts3d.shape}"
        )
        ...
        self.pts_aabb = np.concatenate(
            [np.min(self.pts3d, axis=0), np.max(self.pts3d, axis=0)]
        )  # [6]
        if np.abs(self.pts_aabb).max() > self.opt.bound:
            print(
                f"[WARN] ColmapDataset: estimated AABB {self.pts_aabb.tolist()} exceeds provided bound {self.opt.bound}! Consider improving --bound to make scene included in trainable region."
            )
        ...
        if self.training:
            ...
            for i, k in enumerate(tqdm.tqdm(imkeys)):
                ...
                xys = np.stack(
                    [xys[:, 1], xys[:, 0]], axis=-1
                )  # invert x and y convention...
                pts = imdata[k].point3D_ids

                mask = (
                    (pts != -1)
                    & (xys[:, 0] >= 0)
                    & (xys[:, 0] < camdata[1].height)
                    & (xys[:, 1] >= 0)
                ...
                if self.opt.enable_dense_depth:
                    ...
                    dense_depth = cv2.resize(
                        dense_depth, (self.W, self.H), interpolation=cv2.INTER_LINEAR
                    )
            ...
            self.cam_near_far = torch.from_numpy(
                np.array(self.cam_near_far, dtype=np.float32)
            )  # [N, 2]

            if self.opt.enable_dense_depth:
                self.dense_depth_info = torch.from_numpy(
                    np.stack(self.dense_depth_info, axis=0)
                )
        ...
        self.dodecahedron_poses = torch.from_numpy(
            dodecahedron_poses.astype(np.float32)
        )  # [N, 4, 4]
        self.dodecahedron_mvps = self.projections[[0]] @ torch.inverse(
            self.dodecahedron_poses
        )  # assume the same intrinsic
    ...
    def collate(self, index):
        ...
        if self.training:
            ...
            if self.opt.random_image_batch:
                index = torch.randint(
                    0, len(self.poses), size=(num_rays,), device=self.device
                )
        ...
        if self.images is not None:
            if self.training:
                ...
                if self.opt.enable_dense_depth:
                    depth = (
                        self.dense_depth_info[index, rays["j"], rays["i"]]
                        .float()
                        .to(self.device)
                    )  # [N]
            else:
                images = (
                    self.images[index].squeeze(0).float().to(self.device) / 255
                )  # [H, W, 3/4]
    ...
    def dataloader(self):
        ...
        loader._data = (
            self  # an ugly fix... we need to access error_map & poses in trainer.
        )
