...
class NeRFRenderer(nn.Module):
    ...
    def update_aabb(self, aabb):
        ...
        self.aabb_train = aabb.clamp(-self.real_bound, self.real_bound).to(
            ...
            while head < N:
                ...
                if cam_near_far is None:
                    ...
                    results_ = self.run(
                        rays_o[head:tail],
                        rays_d[head:tail],
                        cam_near_far=cam_near_far[head:tail],
                        **kwargs,
                    )
        ...
        **kwargs,
        ...
        for prop_iter in range(len(self.opt.num_steps)):
            if prop_iter == 0:
                # uniform sampling
                bins = torch.linspace(
                    0, 1, self.opt.num_steps[prop_iter] + 1, device=device
                ).unsqueeze(
                    0
                )  # [1, T+1]
                ...
                # pdf sampling
                bins = sample_pdf(
                    bins, weights, self.opt.num_steps[prop_iter] + 1, perturb
                ).detach()  # [N, T+1]

            real_bins = spacing_fn_inv(
                s_nears * (1 - bins) + s_fars * bins
            )  # [N, T+1] in [near, far]

            rays_t = (real_bins[..., 1:] + real_bins[..., :-1]) / 2  # [N, T]

            xyzs = rays_o.unsqueeze(1) + rays_d.unsqueeze(1) * rays_t.unsqueeze(
                2
            )  # [N, T, 3]
            if self.opt.contract:
                xyzs = contract(xyzs)
            ...
            if not baking and self.opt.background == "last_sample":
                deltas_sigmas = torch.cat(
                    [
                        deltas_sigmas[..., :-1],
                        torch.full_like(deltas_sigmas[..., -1:], torch.inf),
                    ],
                    dim=-1,
                )
            ...
            transmittance = torch.cat(
                [torch.zeros_like(transmittance[..., :1]), transmittance], dim=-1
            )  # [N, T]
    ...
    def mark_untrained_grid(self, dataset, S=64):
        ...
        cam_near_far = (
            dataset.cam_near_far if hasattr(dataset, "cam_near_far") else None
        )  # [B, 2]
        ...
        X = torch.arange(
            self.grid_size, dtype=torch.int32, device=self.aabb_train.device
        ).split(S)
        Y = torch.arange(
            self.grid_size, dtype=torch.int32, device=self.aabb_train.device
        ).split(S)
            ...
            for ys in Y:
                for zs in Z:
                    ...
                    for cas in range(self.cascade):
                        ...
                        mask_min = (
                            cas_world_xyzs >= (self.aabb_train[:3] - half_grid_size)
                        ).sum(-1) == 3
                        mask_max = (
                            cas_world_xyzs <= (self.aabb_train[3:] + half_grid_size)
                        ).sum(-1) == 3
                        ...
                        while head < B:
                            ...
                            cam_xyzs[
                                :, :, 2
                            ] *= -1  # crucial, camera forward is negative now...
                            ...
                            min_near = (
                                self.opt.min_near
                                if cam_near_far is None
                                else cam_near_far[head:tail, 0].unsqueeze(1)
                            )
                            ...
                            mask = (
                                (mask_z & mask_x & mask_y).sum(0).bool().reshape(-1)
                            )  # [N]
