...
def isotropic_explicit_remeshing(verts, faces):
    ...
    print(
        f"[INFO] isotropic explicit remesh: {_ori_vert_shape} --> {verts.shape}, {_ori_face_shape} --> {faces.shape}"
    )
...
def remove_masked_trigs(verts, faces, mask, dilation=5):
    ...
    print(
        f"[INFO] mesh mask trigs: {_ori_vert_shape} --> {verts.shape}, {_ori_face_shape} --> {faces.shape}"
    )
...
def remove_masked_verts(verts, faces, mask):
    ...
    print(
        f"[INFO] mesh mask verts: {_ori_vert_shape} --> {verts.shape}, {_ori_face_shape} --> {faces.shape}"
    )

    return verts, faces


def remove_selected_verts(
    verts,
    faces,
    query="(x < 1) && (x > -1) && (y < 1) && (y > -1) && (z < 1 ) && (z > -1)",
):
    _ori_vert_shape = verts.shape
    _ori_face_shape = faces.shape
    ...
    print(
        f"[INFO] mesh remove verts: {_ori_vert_shape} --> {verts.shape}, {_ori_face_shape} --> {faces.shape}"
    )
...
def decimate_and_refine_mesh(
    verts, faces, mask, decimate_ratio=0.1, refine_size=0.01, refine_remesh_size=0.02
):
    # verts: [N, 3]
    # faces: [M, 3]
    ...
    if decimate_ratio > 0:
        ms.meshing_decimation_quadric_edge_collapse(
            targetfacenum=int((1 - decimate_ratio) * (mask == 1).sum()), selected=True
        )

    if refine_remesh_size > 0:
        ms.meshing_isotropic_explicit_remeshing(
            iterations=3,
            targetlen=pml.AbsoluteValue(refine_remesh_size),
            selectedonly=True,
        )
    ...
    if refine_size > 0:
        ...
        ms.meshing_surface_subdivision_midpoint(
            threshold=pml.AbsoluteValue(refine_size), selected=True
        )
    ...
    print(
        f"[INFO] mesh decimating & subdividing: {_ori_vert_shape} --> {verts.shape}, {_ori_face_shape} --> {faces.shape}"
    )
...
# in meshutils.py
def select_bad_and_flat_faces_by_normal(
    verts, faces, usear=False, aratio=0.02, nfratio_bad=120, nfratio_flat=5
):
    m = pml.Mesh(verts, faces)
    ms = pml.MeshSet()
    ms.add_mesh(m, "mesh")
    ...
    ms.compute_selection_bad_faces(
        usear=usear, aratio=aratio, usenf=True, nfratio=nfratio_bad
    )
    m = ms.current_mesh()
    ...
    ms.compute_selection_bad_faces(
        usear=usear, aratio=aratio, usenf=True, nfratio=nfratio_flat
    )
    m = ms.current_mesh()
